control 'template configuration' do
  ...
  describe file('/etc/template-formula.conf') do
    ...
    its('content') { should include '"tofs": {"files_switch": ["any/path/can/be/used/here", "id", "osfinger", "os", "os_family", "role"], "source_files": {"template-config-file-file-managed": ["example.tmpl.jinja"]}' }
